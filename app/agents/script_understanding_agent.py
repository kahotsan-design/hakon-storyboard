...
6. 【action_beats 动作节拍 —— 与台词同等重要】
   ...
   - 包��但不限于：人物进出场景、人物移动、人物做某个动作、场景变化、物品传递、表情变化、姿态变化。
   - 【表情描写必须提取】剧本中明确写到的表情（如"她眼眶微红""他皱起眉头""苏晚强撑着笑"）必须单独作为一条 action_beat，不可省略、不可并入其他条目。表情是人物情绪的重要视觉信号，丢失表情等于丢失表演指导。
   ...
   - 【严格时间顺序】action_beats 必须按照剧本中出现的先后顺序排列，不可打乱、不可重排。分镜镜头的顺序必须与 action_beats 的顺序一致。
